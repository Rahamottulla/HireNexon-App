          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 items-start">
            {pricingPlans.map((plan, i) => (
              <div key={i} className={`relative rounded-2xl border p-8 flex flex-col transition-all duration-300 ${
                ...
                <div className={`text-sm font-bold uppercase tracking-widest mb-2 ${plan.highlight ? "text-brand-200" : "text-text-muted"}`}>{plan.name}</div>
                <div className="flex items-end gap-1 mb-1">
                  <span className={`text-4xl font-extrabold ${plan.highlight ? "text-white" : "text-text-primary"}`}>{plan.price}</span>
                  <span className={`text-sm font-semibold mb-1.5 ${plan.highlight ? "text-brand-200" : "text-text-muted"}`}>{plan.period}</span>
                </div>
                <p className={`text-xs leading-relaxed mb-6 ${plan.highlight ? "text-brand-200" : "text-text-muted"}`}>{plan.desc}</p>
